...
class Autocomplete extends React.Component {
  ...
  static keyDownHandlers = {
    ArrowDown(event) {
      ...
      let index = highlightedIndex === null || highlightedIndex === itemsLength - 1 ? 0 : highlightedIndex + 1
    ...
    ArrowUp(event) {
      ...
      if (!items.length) return
      const { highlightedIndex } = this.state
      let index = highlightedIndex === 0 || highlightedIndex === null ? items.length -1 : highlightedIndex -1
      for (let i = 0; i < items.length ; i++) {
